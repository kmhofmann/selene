...
TEST_CASE("PNG image reading and writing, no conversion", "[old_img]")
{
...
TEST_CASE("PNG image reading and writing, conversion to grayscale", "[old_img]")
{
  const auto tmp_path = sln_test::get_tmp_path();
  sln::FileReader source(in_filename().string());
  REQUIRE(source.is_open());
...
TEST_CASE("PNG image reading, reusing decompression object", "[old_img]")
{
  const auto tmp_path = sln_test::get_tmp_path();
...
TEST_CASE("PNG image writing, reusing compression object", "[old_img]")
{
  const auto tmp_path = sln_test::get_tmp_path();
...
TEST_CASE("PNG image reading and writing, reading/writing from/to memory", "[old_img]")
{
  const auto tmp_path = sln_test::get_tmp_path();
...
TEST_CASE("PNG reading of the official test suite", "[old_img]")
{
  const auto tmp_path = sln_test::get_tmp_path();
...
TEST_CASE("PNG image reading, through PNGReader interface", "[old_img]")
{
  const auto tmp_path = sln_test::get_tmp_path();
