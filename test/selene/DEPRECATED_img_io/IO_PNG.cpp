...
#include <selene/DEPRECATED_img_io/PNGRead.hpp>
#include <selene/DEPRECATED_img_io/PNGWrite.hpp>
...
void check_write_read(sln::ImageData<>& img_data, const std::filesystem::path& tmp_path)
{
  // Write as PNG file...
...
TEST_CASE("PNG image reading and writing, no conversion", "[DEPRECATED_img]")
{
...
TEST_CASE("PNG image reading and writing, conversion to grayscale", "[DEPRECATED_img]")
{
  const auto tmp_path = sln_test::get_tmp_path();
  sln::FileReader source(in_filename().string());
  REQUIRE(source.is_open());
...
TEST_CASE("PNG image reading, reusing decompression object", "[DEPRECATED_img]")
{
  const auto tmp_path = sln_test::get_tmp_path();
...
TEST_CASE("PNG image writing, reusing compression object", "[DEPRECATED_img]")
{
  const auto tmp_path = sln_test::get_tmp_path();
...
TEST_CASE("PNG image reading and writing, reading/writing from/to memory", "[DEPRECATED_img]")
{
  const auto tmp_path = sln_test::get_tmp_path();
...
TEST_CASE("PNG reading of the official test suite", "[DEPRECATED_img]")
{
  const auto tmp_path = sln_test::get_tmp_path();
...
TEST_CASE("PNG image reading, through PNGReader interface", "[DEPRECATED_img]")
{
  const auto tmp_path = sln_test::get_tmp_path();
