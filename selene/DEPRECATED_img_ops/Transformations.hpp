...
void flip_vertically_in_place(Image<PixelType>& img)
{
  ...
  // TODO: measure
#if 0
  std::vector<PixelType> buffer(DEPRECATED_img.width());  // TODO: could be stack-based in some cases?

  for (auto y_top = 0_px; y_top < half_height; ++y_top)
  {
    const auto y_bottom = PixelIndex{DEPRECATED_img.height() - 1 - y_top};
    std::copy(DEPRECATED_img.data(y_top), DEPRECATED_img.data_row_end(y_top), buffer.begin());
    std::copy(DEPRECATED_img.data(y_bottom), DEPRECATED_img.data_row_end(y_bottom), DEPRECATED_img.data(y_top));
    std::copy(std::cbegin(buffer), std::cend(buffer), DEPRECATED_img.data(y_bottom));
  }
#endif
}
