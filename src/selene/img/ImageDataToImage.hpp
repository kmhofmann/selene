...
Image<PixelType> to_image(ImageData&& img_data)
{
  ...
  const auto data = img_data.relinquish_data_ownership();
  return Image<PixelType>(data, width, height, stride_bytes, true);
}
...
Image<PixelType> to_image_view(ImageData& img_data)
{
  ...
  return Image<PixelType>(data, width, height, stride_bytes, false);
}
